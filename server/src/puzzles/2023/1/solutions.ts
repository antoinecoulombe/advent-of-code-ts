...
39njjvzt7threetkccstz`;
	...
	const replaceAtIndex = (line: string, index: number, replace: string, to: string) => {
		const length = replace.length;
		if (line.substring(index, index + length) === replace) {
			return line.substring(0, index) + to + line.substring(index + length);
		}
		return null;
	};

	const replaceNumberString = (line: string, index: number): string | null => {
		...
		for (const [key, value] of Object.entries(replacementMap)) {
			if (line[index] === key[0]) {
				const newLine = replaceAtIndex(line, index, key, value);
	...
	const replaceNumberStrings = (line: string) => {
		let newLine = line;
		for (let i = 0; i < newLine.length; ++i) {
			const tempLine = replaceNumberString(newLine, i);
			if (tempLine) {
				newLine = tempLine;
				i = 0;
			}
	...
	const part2 = async () => {
		...
		let total = 0;
		lines.forEach((line) => {
			const newLine = replaceNumberStrings(line).replace(/[^0-9]/g, '');
			const first = newLine[0];
